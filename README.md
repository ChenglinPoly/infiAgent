...
- [2026/07/12] **The repository now tracks the InfiAgent backend runtime, installable from PyPI:** `pip install infiagent==3.12.24`. The runtime ships the agent execution loop, context builder, tool executor, the built-in level-0 tool suite, the LLM client with multi-provider failover, the concurrency-safe experience store and the SDK entry point, together with the full test suite. Desktop app, marketplace and other application layers have moved out of this repository; see [RUNTIME.md](./RUNTIME.md) for notes on this build.

- [2026/07/12] **Crash-safe long-horizon execution & concurrency-safe experience store:** All runtime persistence (execution stack, task state, experience files) now uses atomic writes (same-directory temp file + fsync + rename), so power loss can no longer produce half-written state; corrupted legacy state files are automatically quarantined with a clean recovery path, and interrupted tool calls replay idempotently on resume. The experience store gains cross-process file locks covering the full read-modify-write, uuid-based entry ids, file revisions with optimistic concurrency checks (`expected_revision` / `expected_updated_at`), transactional task+global writes with rollback, and only `status=active` entries are injected into agent context.

- [2026/07/12] **Multi-provider LLM failover with cache affinity:** A model entry in `llm_config.yaml` can now declare a `deployments` list — multiple API keys and/or platforms serving the same logical model. Healthy traffic sticks to the primary deployment to preserve provider-side prompt caching; rate limits, timeouts and auth failures put a deployment into cooldown and fail over to the next one automatically, and auth errors no longer abort retries when an alternate deployment is available. The classic single-key configuration remains fully compatible.
