...
global agent_name
agent_name = "answer_from_one_paper"
...
def create_agent(max_turns: int = 100,task_id:str = "default_agent_task") -> Agent:
    ...
    agent_responsibility="你的职责是根据问题从文章中得到答案"
    agent_workflow=f'''
    **你的流程:**
    重要：不要在根目录运行递归的文件展开！！！！
    ...
    1. 你应该根据提供的论文地址，读取论文内容，并根据问题从文章中得到答案，如果有必要提供数学公式，应该使用 latex 风格。你应该输出一个 json 文件，文件名命名规则参考提示词！
    3. 当你认为完成所有任务前，你应该调用 judge_agent 来判断你是否完成任务。你应该给 judge_agent文章地址，和你的总结报告文件地址，而不是文本内容，叫他让其大致检查一下。
    ...
    4. 创建文件前应该检查创建文件的地址下有无其他同名文件，不要覆盖不是自己的文件。文件命名尽量包含你总结的文件名。
    '''
...
if __name__ == '__main__':
    ...
    # 1. 原始指令
    mock_instruction = "文章的地址是/upload/out-of-Order Architecture for Real-Time Data-Driven Resilient Planning and Scheduling of Cyber-Physical Manufacturing Systems.pdf，问题是什么是ut-of-Order Architecture？"
