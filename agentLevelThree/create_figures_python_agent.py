...
global agent_name
agent_name = "create_figures_python_agent"
...
def create_agent(max_turns: int = 100,task_id:str = "default_agent_task") -> Agent:
    ...
    agent_responsibility="你的职责是将用户要求的各种 python作图实现，并输出图片。"
    agent_workflow=f'''
    **你的流程:**
    重要：不要在根目录运行递归的文件展开！！！！
    ...
    2. 使用一个 python 文件在相同目录下进行作图，但是运行时你需要搞清楚，运行工具默认在/code_run 目录下，因此你需要使用../或者其他路径代码正确的读取当前目录下的数据。图片必须是英文的。
    3. 应该使用一个 json 文件或者 md 文件对所做的每一张图（文件名：描述）进行详细描述，并给出文件地址。描述应该经可能详细，比如发现，意义等，有助于后期写作。
    3. 当你认为完成所有任务前，你应该给 judge_agent你的实果的文件地址（如有），你的任务是什么，你的完成结果是什么，请 judge进行判断。需要提示 judge agent 因为运行工具默认在/code_run 目录下，因此你需要使用../或者其他路径代码正确的读取当前目录下的数据。不要使用命令行进行运行，使用封装好的工具进行运行。
    ...
    **严格的输出格式:**
    ...
    available_tools = ['judge_agent','final_output',
                       'dir_list','dir_create','file_read','file_write','pip_install','execute_code']
