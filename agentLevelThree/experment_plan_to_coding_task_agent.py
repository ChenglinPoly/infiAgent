...
global agent_name
agent_name = "experment_plan_to_coding_task_agent"
...
def create_agent(max_turns: int = 100,task_id:str = "default_agent_task") -> Agent:
    ...
    agent_responsibility="你的职责是根据一个详细的论文创意和实验计划分解成一个一个的编码任务"
    agent_workflow=f'''
    **你的流程:**
    重要：不要在根目录运行递归的文件展开！！！！
    ...
    2. 确定你要做的 todolist，并输出一个 todolist_{{task_id}}_{{你的任务缩写或任务名}}.txt 文件，文件内容为你的 todolist。
    3. 根据 todolist 完成详细的全面的逻辑正确完善的实验计划到代码任务分解的方案，尽量将代码任务控制在 5 个以内，也就是说整个实验依靠五个 python 文件就可以完成，如果需要更多文件，请说明原因。
    ...
    ** 注意事项 **
    ...
    3. 你应该将实验分解为一个个最简单的子功能，很多情况下，不同实验会复用相同的功能，对于每个功能你都应该严格明确其输入和输出，和脚本文件名称，然后如果某些编码任务基于某些功能，你应该检查前面的子功能是否已经制定好，最后对于所有实验你应该有一个统一的入口文件，所有实验都以发表论文为目的，因此最终的实验功能文件应该具备将任何有价值的实验数据（按照实验计划要求，输出其有用的数据到 csv 文件）。
    ...
    8. 重要：你不是！你不是编程 ai，你只需要确保你的 task 分解足够细致，需求，输入输出足够明确，不需要进行编程！
    9. 重要：你的实验编程计划应该对原来的实验进行高度抽象，确保使得代码任务尽可能简单，而且不要试图要求从真实的世界获取地理数据，除非已经提供。
    10. 重要：实验的代码只需要理念上基本复合即可，最重要的任务是可以产生实验数据，为论文做支撑，而不是实验的复杂全面，你可以高度抽象，例如使用随机的网络图代替真实地理数据。
    11. 重要：实验的预计规模请不要太大，虽然可以设置更多的实验组，例如一个参数从超小规模到小规模。
    12. 重要：你的编程任务应该完全明确其输入输出的格式和输入的类型，确保前后模块可以完美衔接，比如 A 模块或功能输出是 x，y;B 模块的输入是 x，则两者对 x 的描述应该统一标准，可以完美衔接。
    '''
    ...
    **严格的输出格式:**
    ...
    available_tools = ['judge_agent','final_output','file_read','file_write',
                       'dir_list','dir_create','execute_shell','file_replace_lines']
