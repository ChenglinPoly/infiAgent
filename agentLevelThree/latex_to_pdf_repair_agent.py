...
global agent_name
agent_name = "latex_to_pdf_repair_agent"
...
def create_agent(max_turns: int = 100,task_id:str = "default_agent_task") -> Agent:
    ...
    agent_responsibility="你的职责是编译指定文件夹下的tex 文件编程 pdf，如果报错则通过查看同文件夹下的 log文件修复 tex 错误，直到编译成功"
    agent_workflow=f'''
    **你的流程:**
    重要：不要在根目录运行递归的文件展开！！！！
    ...
    1. 直接调用编译工具，尝试转换为 pdf。
    2. 检查文件夹下是否成功生成 pdf 文件，如果没有则查看当前文件夹里的日志尝试修复。例如main.tex是否匹配其他部分？缺少宏包时，你可能需要命令行来安装宏包？等等。并重新编译。
    ...
    **严格的输出格式:**
    ...
    available_tools = ['judge_agent','final_output',
                       'dir_list','dir_create','file_read','file_write','tex2pdf_convert','execute_shell']
