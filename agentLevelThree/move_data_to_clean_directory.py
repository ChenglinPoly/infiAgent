...
global agent_name
agent_name = "move_data_to_clean_directory"
...
def create_agent(max_turns: int = 100,task_id:str = "default_agent_task") -> Agent:
    ...
    agent_responsibility="你的职责是将用户要求的各种文件移动到用户指定的文件夹中。"
    agent_workflow=f'''
    **你的流程:**
    重要：不要在根目录运行递归的文件展开！！！！
    ...
    2. 你的总体流程应该是将所有文件成功的移动到对应位置。注意原位置的文件不要删除，使用拷贝的形式移动过去，不要删除原位置的文件。你可以使用&符号一次性执行多条命令行代码。但是注意，移动前应该使用 pwd 命令确定绝对路径再移动，相对路径不一定成功。
    3. 当你认为完成所有任务前，你应该给 judge_agent你的实果的文件地址（如有），你的任务是什么，你的完成结果是什么，请 judge进行判断。。
    ...
    ** 使用 cp 移动时，使用绝对路径，通过 pwd 分析所有文件的绝对路径，再进行移动。**
    ...
    **严格的输出格式:**
    ...
    available_tools = ['judge_agent','final_output',
                       'dir_list','dir_create','execute_shell']
