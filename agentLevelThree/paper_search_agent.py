...
def create_agent(max_turns: int = 100,task_id:str = "default_agent_task") -> Agent:
    ...
    agent_responsibility="你的职责是根据任务搜索论文，并返回任务所需论文的完整标题或者doi"
    agent_workflow=f'''
    **你的流程:**
    重要：不要在根目录运行递归的文件展开！！！！
    创建文件前应该使用dir_list工具检查创建文件的地址下有无其他同名文件，不要覆盖不是自己的文件！文件命名尽量独特，根据相关信息进行命名！！！
    在最终输出时，使用 final_output 工具输出你的结果!!!
    0. 你这次任务的 taskid 是{task_id}，你每次调用工具时，都应该将 taskid 作为参数传递给工具。
    1. 使用谷歌学术或者工具中提供的其他论文搜素工具进行搜索，不应该盲目进行搜索，要确保搜素条件尽可能找到所需的论文列表。
    2. 你应该之后通过读取文件的方式读取搜索结果，查看是否有满足要求的论文并记录，你可以在最终输出中输出论文的标题或者doi。
    3，如果搜索结果中有可能满足要求的论文，但是你不确定或者无法读取其 doi 或者完整标题，你应该用打开指定 url 的工具打开搜索列表中对应的 url。同样在之后读取 md 文件获取相关知识。
    4. 当你认为完成所欲任务前，你应该调用 judge_agent 来判断你是否完成任务。你应该给 judge_agent 提供你的初始任务，你找到的论文标题和 doi，以及这些论文的简要介绍。
    无论 judge agent是否通过你都应该进行输出，基于 agent 的意见诚实的输出你的进度，和所有有价值的产出的文件地址和文字产物。并附上 judge agent 的报告作为 output 的一部分，并说明这是来自于judge agent 的报告。
    '''


    agent_output_format='''
         **JSON对象**: 当你准备输出思考过程或做出最终裁决时，必须输出一个严格符合以下格式的JSON字符串,只返回 json 字符串，不要添加任何额外内容，绝对不可以在 content字段输出 tool_calls内容，否则会被我关闭：
        ```
        {{
    ...
    **严格的输出格式:**
    ...
    # 获取可用工具 change_here
    available_tools = ['google_scholar_search','judge_agent','file_read','crawl_page','file_write','dir_list','dir_create','final_output']
...
if __name__ == '__main__':
    ...
    # 1. 原始指令
    mock_instruction = "你找到十篇关于 cyber-physical Internet的文章，最好关于物流相关的，路由相关最好。"
