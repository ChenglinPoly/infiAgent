...
global agent_name
agent_name = "paper_material_collectt"
...
def create_agent(max_turns: int = 100,task_id:str = "default_agent_task") -> Agent:
    ...
    agent_responsibility="你的职责是根据用户想要研究的领域收集相关文献以提供给后续 agent 分析提炼。"
    agent_workflow=f'''
    **你的流程:**
    重要：不要在根目录运行递归的文件展开！！！！
    ...
    1. 使用paper_search_agent 根据用户想要研究的领域收集相关文献，你可以不止一次的调用此工具，每次为其计划要搜索什么子领域的多少篇文献。比如用户想要研究cyber-physical Internet
    的物流问题，你如果计划聚焦于 routing方法上，那么也应该搜索物流中其他的 routing方法作为背景，相关的材料不仅满足后续 idea 产生，也需要满足 related work 的撰写。
    2. 当你收集足够多的 doi 或者标题时，使用另一个工具将文献下载到指定目录，改工具需要一篇篇的输入 title 和 doi，你必须一步步的调用。
    3. 当你认为完成所有任务前，你应该调用 judge_agent 来判断你是否完成任务。你应该给 judge_agent 提供你的初始任务的详细版本也就是你自己最后的规划任务，还有你下载的文件地址，以及这些论文的简要介绍。
    无论 judge agent是否通过你都应该进行输出，基于 agent 的意见诚实的输出你的进度，和所有有价值的产出的文件地址和文字产物。并附上 judge agent 的报告作为 output。并说明这是来自于judge agent 的报告。
    '''
    ...
    **严格的输出格式:**
    ...
    # 获取可用工具 change_here
    available_tools = ['parse_document','paper_search_agent','get_searchPdf_by_doi_or_title','judge_agent','file_read','dir_list','dir_create','final_output']
